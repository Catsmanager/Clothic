...
### 처리 항목
- 이슈 #149 / 작업 브랜치: feat/strip-color-from-item-names (base: develop)
- 아이템 표시 이름에 섞여 있던 색상 단어가 썸네일과 중복되어 정보 정리

### 변경
- constants/itemCatalog.ts — name에서 색상 단어만 제거(18개). 형태/패턴/소재 단어는 유지(스트라이프·데님 등). color 필드는 보존
  - 예: 블랙 블라우스→블라우스, 크림 가디건→가디건, 그레이 플리츠 스커트→플리츠 스커트, 청색 치마→치마
- components/item-select/ItemGrid.tsx — 이름 옆에 color 기반 색 점(스와치) 추가. 색은 데이터에 유지하되 이름에서 분리해 표시
- 적용 범위: 이름은 전 화면(데이터), 색 점은 선택 그리드에만

### 리뷰에서 확인
- name은 표시 전용(아이템 참조/저장은 id) → DB·통계 매칭 영향 없음
- 이름 제거 후에도 카테고리 내 이름 충돌 없음

### 검증
- npx tsc --noEmit → 에러 없음 / npx expo lint → exit 0 / prettier → 정합
