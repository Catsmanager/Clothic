...
- **잠자는 옷장**: outfits에서 item의 마지막 착용일 역산. 아이템 `seasons` 메타데이터로 현재 계절(`all` 포함) 아이템만 대상. 미착용 14~29일 = 관심 필요, 30일+ = 잠자는 옷. 14일 미만·현재 계절 아님·착용 기록 없음은 메인 리스트에서 제외. 현재 계절 판정은 월 기준(3-5 봄/6-8 여름/9-11 가을/12-2 겨울). 계절은 내부 로직 전용(사용자 선택 UI 없음)
