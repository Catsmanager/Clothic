...
- 기본 의상 템플릿에서 실제 보유 의상을 내 옷장에 추가
- 코디 저장 및 날짜별 기록
- 캘린더 조회
...
| 아이템 선택 | `app/item-select.tsx` | 내 옷장 선택·기본 템플릿 빠른 추가 |
...
### 포함
온보딩 · 홈 · 내 옷장 구성 · 코디 생성(상의/하의/신발/가방/액세서리 선택) · 코디 저장 ·
...
| `items` | user_id, catalog_item_id, name, category, color, style_tags, image_path |
...
- 보유를 확인한 옷만 내 옷장·통계·잠자는 옷장에 포함된다.
